...
def run_filtering(args):
    ...
    for i in range(num_imgs):
        ...
        for j in range(num_imgs):
            ...
            #ys
            save = feature_sim.cpu().numpy()
            image = Image.fromarray((save*256).astype(np.int8))
            image.save('/data/yisi/mywork/omnimotion/my_image.png') 

            feature_mask = feature_sim > 0.5

            #ys
            save2 =  feature_mask.cpu().numpy()
            image2 = Image.fromarray(save2.astype(np.int8))
            image2.save('/data/yisi/mywork/omnimotion/my_image2.png')
