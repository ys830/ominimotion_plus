...
def run(args):
    ...
    imgs = []
    for i in img_files:
        imgs.append(int(i.split(".")[0].split("/")[-1]) )
    ...
    for i in range(len(imgs) - 1):
        imgname_i = str(imgs[i]).zfill(5)+'.jpg'
        imgname_i_plus_1 = str(imgs[i+1]).zfill(5)+'.jpg'
        ...
        feature = torch.from_numpy(np.load(os.path.join(scene_dir, 'features', feature_name,
                                        imgname_i + '.npy'))).float().to(DEVICE)
        feature_i = feature.permute(2, 0, 1)[None]
        ...
        for j in range(i + 1, len(imgs)):
            # # vis
            imgname_j = str(imgs[j]).zfill(5)+'.jpg'
            ...
            feature = torch.from_numpy(np.load(os.path.join(scene_dir, 'features', feature_name,
                                        imgname_j + '.npy'))).float().to(DEVICE)
            feature_j = feature.permute(2, 0, 1)[None]
            ...
            image=torch.from_numpy(imageio.imread(os.path.join(scene_dir, 'color',imgname_j)) / 255.).float().permute(2, 0, 1)[None].to(DEVICE)
            image_j_sampled = F.grid_sample(image, curr_coords_normed, align_corners=True).squeeze()
            image=torch.from_numpy(imageio.imread(os.path.join(scene_dir, 'color',imgname_i)) / 255.).float().permute(2, 0, 1)[None].to(DEVICE)
            rgb_sim = torch.norm(image - image_j_sampled, dim=1).squeeze(0).cpu().numpy()
            ...
            imgname_j_plus_1 = str(imgs[j+1]).zfill(5)+'.jpg'
    ...
    for i in range(len(imgs) - 1, 0, -1):
        imgname_i = str(imgs[i]).zfill(5)+'.jpg'
        imgname_i_minus_1 = str(imgs[i-1]).zfill(5)+'.jpg'
        ...
        for j in range(i - 1, -1, -1):
            ...
            imgname_j_minus_1 = str(imgs[j-1]).zfill(5)+'.jpg'
    ...
    for i in range(len(imgs)):
        img_name = str(imgs[i]).zfill(5)+'.jpg'
