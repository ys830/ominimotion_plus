...
class CouplingLayer(nn.Module):
    ...
    def forward(self, F, y):
        ...
        x = y1 + (1 - self.mask) * ((y - t) * torch.exp(-s)) #[8,256,32,3]
        ldj = (-s).sum(-1) #[8,256,32]
...
class NVPSimplified(nn.Module):
    def __init__(
        ...
        for i in self.layer_idx:
            ...
            # map_st = networks.hashsiren.HashSiren(
            #     in_features=  proj_dims + feature_dims,
            #     hidden_features=256,
            #     hidden_layers=4,
            #     out_features=2
            # )
    ...
    def _affine_input(self, t, x, inverse=False):
        ...
        net_in = torch.stack([t[..., None].repeat(1, *x.shape[1:3]), depth], dim=-1) #[x,y]
