...
class BaseTrainer():
    ...
    def get_prediction_one_way(self, x, id, inverse=False):
        ...
        if inverse:
            ...
            out = self.deform_mlp.forward(t, feature, x) # 6 affine coupling layer
    ...
    def compute_scene_flow_reg_loss(self, ids, xs):
        mask_valid = (ids >= 1) * (ids < self.num_imgs - 1)
        ids = ids[mask_valid]
        if len(ids) == 0:
            return torch.tensor(0.)
        ...
        xs_prev_after = self.get_predictions(torch.cat([xs, xs], dim=0),
                                             ...
                                             np.concatenate([ids_prev, ids_after]))
        xs_prev, xs_after = torch.split(xs_prev_after, split_size_or_sections=len(xs), dim=0)
        loss = masked_l1_loss(xs, xs_prev) + masked_l1_loss(xs, xs_after)
        return loss
    ...
    def compute_all_losses(self,
        ...
        mask_valid = (ids1 >= 1) * (ids1 < self.num_imgs - 1)

        # 对px1s计算前后两帧
        x1s_canonical_samples = self.get_prediction_one_way(x1s_samples, ids1)
        x1s_samples_prev, x1s_samples_after, xs_samples_prev_canonical, xs_samples_after_canonical = self.compute_bio_scene_coor(x1s_samples,ids1)
        out = self.get_blending_weights(x1s_canonical_samples)
        blending_weights1 = out['weights']
        alphas1 = out['alphas']
        pred_rgb1 = out['rendered_rgbs']

        mask = (x1s_samples[..., -1] >= depth_min_th) * (x1s_samples[..., -1] <= depth_max_th)
        blending_weights1 = blending_weights1 * mask.float()

        loss_rgb1 = F.mse_loss(pred_rgb1[rgb_mask], gt_rgb1[rgb_mask])
        loss_rgb1_grad = self.gradient_loss(pred_rgb1[rgb_mask], gt_rgb1[rgb_mask])
        ...
        x1s_prev_proj_samples, x1s_prev_proj_depth_samples = self.project(x1s_samples_prev, return_depth=True)
        ...
        if mask_prev.sum() > 0:
            loss_rgb1_prev = F.mse_loss(pred_rgb1_prev[rgb_mask_prev], gt_rgb1[mask_valid][rgb_mask_prev])
            loss_rgb1_grad_prev = self.gradient_loss(pred_rgb1_prev[rgb_mask_prev], gt_rgb1[mask_valid][rgb_mask_prev])

            #计算coor的loss
            coor_loss_prev = self.compute_coor_loss(xs_samples_prev_canonical, x1s_samples[mask_valid],ids1[mask_valid])

            # mapped depth should be within the predefined range
            # depth_range_loss_prev = compute_depth_range_loss(x1s_prev_proj_depth_samples, depth_min_th, depth_max_th)

            # loss_prev = w_rgb *(loss_rgb1_prev + loss_rgb1_grad_prev) + coor_loss_prev + w_depth_range *depth_range_loss_prev
        ...
        # [n_imgs, n_pts, n_samples, 2]
        x1s_after_proj_samples, x1s_after_proj_depth_samples = self.project(x1s_samples_after, return_depth=True)
        ...
        if mask_after.sum() > 0:
            loss_rgb1_after = F.mse_loss(pred_rgb1_after[rgb_mask_after], gt_rgb1[mask_valid][rgb_mask_after])
            loss_rgb1_grad_after = self.gradient_loss(pred_rgb1_after[rgb_mask_after], gt_rgb1[mask_valid][rgb_mask_after])

            #计算coor的loss
            coor_loss_after = self.compute_coor_loss(xs_samples_after_canonical, x1s_samples[mask_valid],ids1[mask_valid])

            # mapped depth should be within the predefined range
            # depth_range_loss_after = compute_depth_range_loss(x1s_after_proj_depth_samples, depth_min_th, depth_max_th)

            # loss_after = w_rgb * (loss_rgb1_after + loss_rgb1_grad_after) + coor_loss_after + w_depth_range *depth_range_loss_after
        ...
        # only apply to 25% of points to reduce cos
        # scene_flow_smoothness_loss = self.compute_scene_flow_smoothness_loss(ids1, x1s_samples[:, :int(num_pts / 4)])
        # scene_flow_smoothness_loss = self.compute_scene_flow_smoothness_loss(ids1, x1s_samples)
        ...
        #针对单向的正则项
        # reg_loss = self.compute_scene_flow_reg_loss(ids1, x1s_samples)
        
        # loss = loss_prev + loss_after + w_rgb * (loss_rgb1 + loss_rgb1_grad) + w_canonical_unit_sphere *canonical_unit_sphere_loss + \
        #       w_scene_flow_smooth *scene_flow_smoothness_loss + w_distortion * distortion_loss
      
        loss = loss_prev + loss_after + loss_rgb1 + loss_rgb1_grad
        
        if write_logs:
            self.scalars_to_log['{}/Loss'.format(log_prefix)] = loss.item()
            self.scalars_to_log['{}/loss_after'.format(log_prefix)] =loss_after.item()
            self.scalars_to_log['{}/loss_prev'.format(log_prefix)] =loss_prev.item() 
            self.scalars_to_log['{}/loss_rgb1'.format(log_prefix)] =loss_rgb1.item()
            self.scalars_to_log['{}/loss_rgb1_prev'.format(log_prefix)] =loss_rgb1_prev.item()
            # self.scalars_to_log['{}/canonical_unit_sphere_loss'.format(log_prefix)] =canonical_unit_sphere_loss.item()
            # self.scalars_to_log['{}/scene_flow_smoothness_loss'.format(log_prefix)] =scene_flow_smoothness_loss.item() 
            # self.scalars_to_log['{}/distortion_loss'.format(log_prefix)] =distortion_loss.item() 
            self.scalars_to_log['{}/coor_loss_prev'.format(log_prefix)] =coor_loss_prev.item() 
            # self.scalars_to_log['{}/reg_loss'.format(log_prefix)] =reg_loss.item()       
    ...
    def eval_video_correspondences(self, query_id, pts=None, num_pts=200, seed=1234, use_mask=False,
        ...
        with torch.no_grad():
            if mask is not None:
                ...
                mask = self.masks[query_id] #[N.,480,853]
    ...
    def log(self, writer, step):
        if self.args.local_rank == 0:
            ...
            #     flows = self.get_pred_flows(self.ids1[0:1], self.ids1[0:1]+1, chunk_size=self.args.chunk_size)[0]
            ...
            #     ids = np.concatenate([self.ids1[0:1], self.ids1[0:1]+1])
            #     with torch.no_grad():
